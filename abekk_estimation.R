...
#
# This script estimates an Asymmetric BEKK(1,1,1) model to analyze volatility
# spillovers between markets. The ABEKK model captures:
# - Volatility clustering (GARCH effects)
# - Shock transmission (ARCH effects)
# - Asymmetric responses to negative shocks (leverage effects)
...
# Note: Assumes 'returns' object exists in workspace
# The returns object should be an xts/zoo/matrix with columns for each market
# Example markets: US, CN, EU (adjust based on your data)
...
# Asymmetric BEKK specification
# The model: H_t = C'C + A' * ε_{t-1} * ε'_{t-1} * A + G' * H_{t-1} * G
#                      + B' * η_{t-1} * η'_{t-1} * B
# where η_t = min(ε_t, 0) captures negative shocks (asymmetric term)
...
# Check convergence
if (asymmetric_bekk_fit$estimation_time > 0) {
  cat(sprintf("Estimation completed successfully in %.2f seconds\n",
              asymmetric_bekk_fit$estimation_time))
  cat(sprintf("Log-likelihood: %.4f\n", asymmetric_bekk_fit$log_likelihood))
  cat(sprintf("AIC: %.4f\n", asymmetric_bekk_fit$AIC))
  cat(sprintf("BIC: %.4f\n\n", asymmetric_bekk_fit$BIC))
} else {
  warning("Estimation may not have converged. Check results carefully.")
}
...
# A: ARCH matrix (short-run shock spillovers)
# B: Asymmetric matrix (negative shock spillovers/leverage effects)
# G: GARCH matrix (long-run volatility spillovers/persistence)

C_matrix <- params$C
A_matrix <- params$A
B_matrix <- params$B  # Asymmetric/leverage effects
G_matrix <- params$G
...
cat("(Lower triangular Cholesky factor)\n")
print(C_matrix)
cat("\n")
...
# Extract standard errors and t-statistics if available
if (!is.null(asymmetric_bekk_fit$QML_t_ratios)) {
  cat("T-ratios for parameter estimates:\n")
  print(asymmetric_bekk_fit$QML_t_ratios)
  cat("\n")
} else {
  cat("T-ratios not available. Rerun with QML_t_ratios = TRUE\n\n")
}
...
# Calculate total spillover indices
# Total spillover = |A[i,j]| + |B[i,j]| + |G[i,j]|
...
cat("Own-market leverage effects (diagonal of B):\n")
for (i in 1:n_markets) {
  cat(sprintf("%s: %.4f (t-stat: %s)\n",
              market_names[i],
              B_matrix[i,i],
              ifelse(!is.null(asymmetric_bekk_fit$QML_t_ratios),
                     sprintf("%.2f", asymmetric_bekk_fit$QML_t_ratios$B[i,i]),
                     "N/A")))
}
cat("\n")
...
latex_combined <- paste0(
  "\\documentclass{article}\n",
  "\\usepackage{booktabs}\n",
  "\\usepackage{caption}\n",
  "\\usepackage{float}\n",
  "\\begin{document}\n\n",
  "\\section{ABEKK Model Results}\n\n",
  "\\subsection{Model Summary}\n",
  print(latex_summary, print.results = FALSE,
        include.rownames = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\subsection{Parameter Estimates}\n\n",
  "\\subsubsection{Constant Matrix}\n",
  print(latex_C, print.results = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\subsubsection{ARCH Matrix (Short-run Spillovers)}\n",
  print(latex_A, print.results = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\subsubsection{Asymmetric Matrix (Leverage Effects)}\n",
  print(latex_B, print.results = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\subsubsection{GARCH Matrix (Long-run Spillovers)}\n",
  print(latex_G, print.results = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\subsection{Spillover Analysis}\n",
  print(latex_spillover, print.results = FALSE, booktabs = TRUE,
        caption.placement = "top"),
  "\n\\end{document}"
)

writeLines(latex_combined, "output/abekk_results_complete.tex")
cat("Complete LaTeX document saved to: output/abekk_results_complete.tex\n\n")
...
if (!is.null(asymmetric_bekk_fit$H_t)) {
  ...
  # Reshape for ggplot
  vol_long <- reshape2::melt(vol_data, id.vars = "Date",
                            variable.name = "Market",
                            value.name = "Volatility")

  # Create volatility plot
  p1 <- ggplot(vol_long, aes(x = Date, y = Volatility, color = Market)) +
...
# Plot 3: Directional spillovers bar chart
cat("Generating directional spillover chart...\n")
...
results <- list(
  ...
  data_info = list(
    ...
    date_range = c(min(index(returns)), max(index(returns)))
  )
)
...
spillover_vec[spillover_vec == 0] <- NA
top_spillovers <- sort(spillover_vec, decreasing = TRUE, na.last = TRUE)[1:5]
for (val in top_spillovers) {
  idx <- which(spillover_matrix == val, arr.ind = TRUE)
  if (nrow(idx) > 0) {
    cat(sprintf("- %s → %s: %.4f\n",
                market_names[idx[1,2]],
                market_names[idx[1,1]],
                val))
  }
}
...
cat("    - table_spillover.tex\n")
cat("    - table_summary.tex\n")
cat("    - abekk_results_complete.tex (full document)\n\n")
cat("  Visualizations:\n")
...
cat("3. Or compile full document: pdflatex output/abekk_results_complete.tex\n\n")
